...
class Entity(Page):
    ...
    creator_text = models.CharField(
        max_length=128, blank=True, null=True, verbose_name="Author/Creator (Text)"
    )

    subtype = models.CharField(
        max_length=128, blank=True, null=True, verbose_name="Category/Role"
    )
    date_from = models.CharField(
        max_length=128, blank=True, null=True, verbose_name="Date 1"
    )
    date_to = models.CharField(
        max_length=128, blank=True, null=True, verbose_name="Date 2"
    )

    date_mozart = models.CharField(
        max_length=128, blank=True, null=True, verbose_name="Date related to mozart"
    )

    # Normally wouldn't use a charfield for this, but
    # looking at the example data that's what's needed
    location = models.CharField(
        max_length=256, null=True, blank=True, verbose_name="Location"
    )
    sublocation = models.CharField(
        max_length=256, null=True, blank=True, verbose_name="Sublocation"
    )
    location_mozart = models.CharField(
        max_length=256,
        null=True,
        blank=True,
    ...
    mozart_relevence = StreamField(
        CMSStreamBlock(required=False),
        verbose_name="Mozart Relevance",
        null=True,
        blank=True,
    )

    location_purchase = models.CharField(
        max_length=256, null=True, blank=True, verbose_name="Place of Purchase"
    )

    bibliog = models.TextField(
        null=True, blank=True, verbose_name="Bibliographic Reference"
    )

    entity_comments = models.TextField(null=True, blank=True, verbose_name="Comments")

    search_fields = Page.search_fields + [
        index.SearchField("body"),
        index.SearchField("subtype"),
        index.SearchField("mozart_relevence"),
        index.SearchField("entity_comments"),
    ]
...
Entity.content_panels = [
    ...
    FieldPanel("entity_comments", classname="full"),
        ...
        try:
            ...
            if isinstance(self.get_parent().specific, ObjectIndexPage):
                ...
                if num < self.get_parent().get_children().count():
                    ...
                    parent_num = (
                        list(parent_page.get_parent().get_children()).index(parent_page)
                        + 1  # noqa
                    )
...
class EntityThrough(models.Model):
    entity = models.ForeignKey(
        Entity, on_delete=models.CASCADE, related_name="throughentity"
    )
    page = models.ForeignKey(Page, on_delete=models.CASCADE, related_name="throughpage")
